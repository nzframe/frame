...
class ExportAPP(App):
    ...
    def output_data(self, file_handler):
        for e in self.get_data():
            ...
            rt.append(e[2])
            rt.append(e[3])
            print(f"create_wall({rt[0]}, {rt[1]}, {rt[2]}, {rt[3]}, {rt[4]}, {rt[5]}, {rt[6]}, {rt[7]})")
            print(f"create_wall({float(rt[0])*304.8}, {rt[1]}, {rt[2]}, {float(rt[3])*304.8}, {rt[4]}, {rt[5]}, {float(rt[6])*304.8}, {float(rt[7])*304.8})")
